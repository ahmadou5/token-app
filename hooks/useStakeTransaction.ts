...
export function useStakeTransaction() {
  ...
  const executeStakeAction = useCallback(
    async (
      ...
      try {
        ...
        const [{ TransactionBuilder }, { address }] = await Promise.all([
          import("@pipeit/core"),
          import("@solana/kit"),
        ]);
        ...
        const res = await fetch("/api/stake/transaction", {
          ...
          body: JSON.stringify({
            ...
            amountLamports: params.amountSOL
              ? Math.floor(params.amountSOL * 1e9)
              : undefined,
        ...
        if (!data.ok)
          throw new Error(data.err || "Failed to build transaction");
  ...
  const fetchActiveStakes = useCallback(
    async (wallet: string): Promise<StakePosition[]> => {
      if (!client) return [];
      try {
        const { rpc } = client;
        ...
        const res = await rpc
          .getProgramAccounts(kitAddress(STAKE_PROGRAM_ID), {
            filters: [
              { dataSize: 200n },
              {
                memcmp: {
