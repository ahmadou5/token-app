...
function formatSolFull(sol: number): string {
  if (sol >= 1_000_000) return (sol / 1_000_000).toFixed(4) + "M SOL";
    ...
    >
      <div
        style={{
          ...
          border: "1px solid var(--tc-tooltip-border)",
          borderRadius: 8,
          padding: "8px 12px",
          minWidth: 140,
          boxShadow: "var(--tc-shadow-hover)",
          whiteSpace: "nowrap",
        }}
      >
        <div
      ...
      <div
        style={{
          ...
          borderTop: "5px solid var(--tc-tooltip-border)",
        }}
      />
    </div>
  );
...
function TotalStakeChart({
  ...
  const [tip, setTip] = useState<TipState>({ visible: false, x: 0, y: 0, epoch: 0, primary: "" });
  ...
  const derived = useMemo(() => {
    ...
    const xLabels = Array.from({ length: Math.min(5, data.length) }, (_, i) => {
      ...
      return { x: PAD.left + (idx / (data.length - 1)) * iW, label: `Ep ${data[idx].epoch}` };
    });

    return { pts, path, areaPath, yLabels, xLabels, pulsePt: last, iW };
  }, [data]);
  ...
  const onMouseMove = useCallback(
    (e: React.MouseEvent<SVGSVGElement>) => {
      ...
      const frac = Math.max(0, Math.min(1, (svgX - PAD.left) / derived.iW));
      const idx = Math.round(frac * (data.length - 1));
      const [px, py] = derived.pts[idx];
      setTip({
        visible: true,
        x: (px / W) * rect.width,
        y: (py / H) * rect.height,
        epoch: data[idx].epoch,
        primary: formatSolFull(data[idx].value),
        secondary: "Cumulative activated stake",
      });
    },
    [derived, data],
  );
  ...
  const { pts, path, areaPath, yLabels, xLabels, pulsePt } = derived;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {yLabels.map(({ y }) => (
          <line key={y} x1={PAD.left} y1={y} x2={W - PAD.right} y2={y}
        ...
        {yLabels.map(({ y, label }) => (
          <text key={y} x={PAD.left - 8} y={y + 4} textAnchor="end"
            fontSize="10" fill="var(--tc-text-muted)" fontFamily="var(--tc-font-mono)">
            {label}
          </text>
        ))}
        {xLabels.map(({ x, label }) => (
          <text key={x} x={x} y={H - 8} textAnchor="middle"
            fontSize="10" fill="var(--tc-text-muted)">
            {label}
          </text>
        ))}
        ...
        {animated && (
          <g>
            <circle cx={pulsePt[0]} cy={pulsePt[1]} r="14"
              fill={lineColor} opacity="0" className="td-chart__pulse-ring" />
            <circle cx={pulsePt[0]} cy={pulsePt[1]} r="4"
              fill={lineColor} className="td-chart__pulse-dot" />
          </g>
        )}

        {/* full-width invisible hover target */}
...
function FlowChart({ data }: { data: HistoryPoint[] }) {
  ...
  const derived = useMemo(() => {
    ...
    const bars = data.map((d, i) => {
      ...
      return {
        x: PAD.left + i * slotW + slotW * 0.175,
        cx: PAD.left + i * slotW + slotW / 2,
        topY: PAD.top + iH - barH,
        barH,
        sol,
        epoch: d.epoch,
      };
    ...
    const xLabels = Array.from({ length: Math.min(5, data.length) }, (_, i) => {
      const idx = Math.round((i / 4) * (data.length - 1));
      return { x: PAD.left + idx * slotW + slotW / 2, label: `Ep ${data[idx].epoch}` };
    });

    return { bars, barW, yLabels, xLabels, iH, maxVal, slotW };
  }, [data]);
  ...
  const onMouseMove = useCallback(
    (e: React.MouseEvent<SVGSVGElement>) => {
      ...
      const idx = Math.max(
        0,
        Math.min(data.length - 1, Math.floor((svgX - PAD.left) / derived.slotW)),
      );
      const bar = derived.bars[idx];
      const pct = ((bar.sol / derived.maxVal) * 100).toFixed(1);
      setHoveredEpoch(bar.epoch);
      setTip({
        visible: true,
        x: (bar.cx / W) * rect.width,
        y: (bar.topY / H) * rect.height,
        epoch: bar.epoch,
        primary: formatSolFull(bar.sol),
        secondary: `${pct}% of peak epoch flow`,
      });
    },
    [derived, data],
  );
...
function StakeChart({
  ...
  return (
    <>
      <div className="td-chart-controls" style={{ marginBottom: 10, flexWrap: "nowrap", alignItems: "center" }}>
        ...
        <div
          style={{
            ...
            fontSize: 11, color: "var(--tc-text-muted)",
            marginLeft: "auto", flexShrink: 0,
          }}
        >
          <div style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--tc-accent)", animation: "tgPulse 1.5s ease-in-out infinite" }} />
          Live
        </div>
      </div>
      ...
      <p style={{ margin: "6px 0 0", fontSize: 10, color: "var(--tc-text-muted)", fontStyle: "italic", lineHeight: 1.5 }}>
        {view === "total"
          ? "Cumulative stake anchored to current activated stake. Hover for epoch details."
          : "Per-epoch delegation flow in SOL. Hover a bar for details. Spikes = large delegation events."}
      </p>
    </>
  );
}
...
export function ValidatorDetailContent({ validator }: ValidatorDetailContentProps) {
  ...
  const solana = tokens.find((t) => t.assetId === "solana");
  ...
  const handleMax = () => { if (solBalance) setAmount(Math.max(0, solBalance - 0.01).toFixed(4)); };
  const handleStake = async () => {
    const val = parseFloat(amount);
    if (!val || val <= 0) return;
    await executeStakeAction("stake", { voteAccount: validator.votingPubkey, amountSOL: val });
  };
  const handleUnstake = async (stakeAccount: string) => executeStakeAction("deactivate", { stakeAccount });
  const handleWithdraw = async (stakeAccount: string, amt: number) => executeStakeAction("withdraw", { stakeAccount, amountSOL: amt });

  const estimatedDaily = useMemo(() => (parseFloat(amount) || 0) * (validator.apy / 100) / 365, [amount, validator.apy]);
      ...
      <div>
        <div className="td-header">
          <div style={{ position: "relative", flexShrink: 0 }}>
            {validator.avatar ? (
              <img src={validator.avatar} alt={validator.name} className="tc-avatar"
            ...
            {validator.status === "active" && (
              <div style={{ position: "absolute", bottom: -4, right: -4, background: "var(--tc-accent-up)", borderRadius: "50%", padding: 3, border: "2px solid var(--tc-bg)", display: "flex" }}>
          ...
          <div className="td-header__info">
            <div className="td-header__row">
              ...
              {validator.isJito && <span className="tc-badge tc-badge--t1">Jito MEV</span>}
            </div>
            <div className="td-header__pills">
              <button className="td-pill td-pill--mint"
              ...
              {validator.website && (
                <a href={validator.website} target="_blank" rel="noopener noreferrer" className="td-pill">
          ...
          <div className="td-header__actions">
            <div className="tc-stat">
              ...
              <span className="tc-stat__value" style={{ fontSize: 18, fontWeight: 700 }}>{validator.commission}%</span>
            </div>
            <div style={{ width: 1, height: 36, background: "var(--tc-divider)", margin: "0 12px" }} />
            <div className="tc-stat">
              <span className="tc-stat__label">Yield APY</span>
              <span className="tc-stat__value" style={{ fontSize: 18, fontWeight: 700, color: "var(--tc-accent-up)" }}>
        ...
        <div className="td-section">
          ...
          <div className="td-card">
            ...
            <div style={{ marginTop: 20, paddingTop: 20, borderTop: "1px solid var(--tc-divider)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
              <div className="tc-stat">
                <span className="tc-stat__label" style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 8 }}>
                  <ClockCounterClockwise size={13} /> Epoch Credits
                </span>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {validator.epochCredits && validator.epochCredits.length > 0 ? (
                    validator.epochCredits.slice(-5).reverse().map((credits, idx) => (
                      <div key={idx} style={{ background: "var(--tc-surface)", border: "1px solid var(--tc-border)", borderRadius: 8, padding: "4px 8px", display: "flex", flexDirection: "column", alignItems: "center" }}>
                        <span style={{ fontSize: 9, color: "var(--tc-text-muted)", fontWeight: 600, textTransform: "uppercase" }}>
                          {idx === 0 ? "NOW" : `E-${idx}`}
                        </span>
                        <span className="tc-stat__value" style={{ fontSize: 12 }}>{(credits / 1000).toFixed(0)}k</span>
                      </div>
                    ))
                  ) : (
                    <span style={{ fontSize: 12, color: "var(--tc-text-muted)", fontStyle: "italic" }}>Syncing…</span>
                  )}
                </div>
              </div>
              ...
              <div className="tc-stat">
                <span className="tc-stat__label" style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 8 }}>
                  <ShieldCheck size={13} /> Network Position
                </span>
                <p style={{ margin: 0, fontSize: 13, color: "var(--tc-text-secondary)", lineHeight: 1.6 }}>
                  Ranked <strong style={{ color: "var(--tc-text-primary)" }}>#{validator.rank}</strong> globally by stake weight. Contributing to network decentralization.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="td-stats-grid">
          <div className="td-stat-cell"><span className="td-stat-cell__label">Version</span><span className="td-stat-cell__value">{validator.version || "1.18.x"}</span></div>
          <div className="td-stat-cell"><span className="td-stat-cell__label">Uptime</span><span className="td-stat-cell__value">{validator.uptime ? `${(validator.uptime * 100).toFixed(2)}%` : "99.99%"}</span></div>
          <div className="td-stat-cell"><span className="td-stat-cell__label">City</span><span className="td-stat-cell__value">{validator.city || "—"}</span></div>
          <div className="td-stat-cell"><span className="td-stat-cell__label">Country</span><span className="td-stat-cell__value">{validator.country || "International"}</span></div>
          <div className="td-stat-cell" style={{ gridColumn: "span 2" }}><span className="td-stat-cell__label">Data Center</span><span className="td-stat-cell__value">{validator.dataCenter || "Global Infrastructure"}</span></div>
        </div>
      </div>

      {/* RIGHT */}
      <div className="td-sidebar">
        <div className="h-[54px] mb-[26px]">
          {isConnected && (
        ...
        <div className="sw-card">
          <div className="sw-tabs">
            <div className="sw-tab sw-tab--active">
              <Lightning size={12} weight="fill" /> Native Staking
            </div>
          </div>
          ...
          <div style={{ padding: "14px 14px 12px", display: "flex", flexDirection: "column", gap: 14 }}>
            {existingStake ? (
              <>
                <div style={{ background: "var(--tc-surface)", border: "1px solid var(--tc-border)", borderRadius: 10, padding: "12px 14px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                    <span className="sw-input-lbl">Active Stake</span>
                    <span className="tc-change tc-change--up tc-change--sm">{existingStake.status}</span>
                  </div>
                  <span className="sw-amount sw-amount--out" style={{ display: "block" }}>
                    {existingStake.amount}
                    <span style={{ fontSize: 13, color: "var(--tc-text-muted)", marginLeft: 4 }}>SOL</span>
                  </span>
                </div>
                {existingStake.status === "active" ? (
                  <button className="sw-swap-btn"
                    style={{ width: "100%", margin: 0, background: "transparent", border: "1.5px solid var(--tc-accent-down)", color: "var(--tc-accent-down)" }}
                    onClick={() => handleUnstake(existingStake.address)} disabled={isExecuting}>
                    {isExecuting ? "Processing…" : "Unstake SOL"}
                  </button>
                ) : (
                  <button className={`sw-swap-btn ${isExecuting ? "sw-swap-btn--busy" : ""}`}
                    style={{ width: "100%", margin: 0 }}
                    onClick={() => handleWithdraw(existingStake.address, existingStake.amount)} disabled={isExecuting}>
              ...
              <>
                <div className="sw-input-group" style={{ padding: 0 }}>
                  <div className="sw-input-hdr">
                    ...
                    {isConnected && solBalance != null && (
                      <button className="sw-bal-btn" onClick={handleMax}>MAX: {solBalance.toFixed(3)} SOL</button>
                    )}
                  </div>
                  <div className="sw-input-row">
                    <input type="number" min="0" placeholder="0.00" className="sw-amount"
                      value={amount} onChange={(e) => setAmount(e.target.value)} />
                    <div className="sw-token-fixed">
                      <img className="w-5 h-5 rounded-xl" src={solana?.imageUrl || ""} alt={solana?.name} />
                    </div>
                  </div>
                </div>
                ...
                {parseFloat(amount) > 0 && (
                  <div className="sw-quote" style={{ margin: 0 }}>
                    <div className="sw-quote__row">
                      ...
                      <span className="sw-quote__val" style={{ color: "var(--tc-accent-up)" }}>+{estimatedDaily.toFixed(4)} SOL</span>
                    </div>
                    <div className="sw-quote__row">
                      <span className="sw-quote__label">APY</span>
                      <span className="sw-quote__val" style={{ color: "var(--tc-accent-up)" }}>{validator.apy.toFixed(2)}%</span>
                    </div>
                    <div className="sw-quote__row sw-quote__row--provider">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <img className="w-5 h-5 rounded-full" src={validator.avatar} alt={validator.name} />
                      </div>
                    </div>
                  </div>
                )}
                ...
                <button className={`sw-swap-btn ${isExecuting ? "sw-swap-btn--busy" : ""}`}
                  style={{ width: "100%", margin: 0 }}
                  disabled={!isConnected || isExecuting || !amount} onClick={handleStake}>
                  {isExecuting && <span className="sw-spinner" />}
                  {!isConnected ? "Connect Wallet" : isExecuting ? "Signing…" : "Stake SOL"}
                </button>
              </>
            )}
            <p className="sw-powered" style={{ padding: 0, margin: 0 }}>Secured by Solana Native Staking</p>
          </div>
        </div>
        ...
        <div className="td-card">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              ...
              { label: "Uptime",       value: validator.uptime ? `${(validator.uptime * 100).toFixed(1)}%` : "99.9%" },
              { label: "Active Stake", value: formatSol(currentStakeSol) + " SOL" },
            ].map(({ label, value, color }) => (
              <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 10, borderBottom: "1px solid var(--tc-divider)" }}>
                <span className="tc-stat__label" style={{ textTransform: "none", fontSize: 12 }}>{label}</span>
                <span className="tc-stat__value" style={{ color: color || "var(--tc-text-primary)", fontSize: 13 }}>{value}</span>
              </div>
            ))}
          </div>
        </div>
